...
// 古本市の予定欄を表示する関数
function show_bookfair_info_of_all(){
	$bookfair_infos = get_bookfair_info_of_all();
	echo "<table>
            <tr>
                <th>開催日</th>
                <th>開始時間</th>
        	...
        	foreach($bookfair_infos as $bookfair_info){
            	...
            	echo "
            	<tr>
	                <td data-date=''> " . date('Y/m/d',$start_datetime) . "</td>
	                <td data-start=''> " . date('H:i',$start_datetime) . "</td>
	                <td data-end=''> " . date('H:i',$end_datetime) . "</td>
	                <td data-venue=''> " . $bookfair_info->venue . "</td>
